...
#define aGLBufferDestroy(b) do{AGL__CALL(glDeleteBuffers(1,&(b)->name);(b)->name=0);}while(0)
...
void aGLTextureUpload(AGLTexture* tex, const AGLTextureUploadData* data) {
	...
	if (data->x || data->y) {
		...
		AGL__CALL(glTexSubImage2D(GL_TEXTURE_2D, 0, data->x, data->y, data->width, data->height,
	...
	const AGLDrawTarget *target)
{
	...
	if (src->primitive.index.buffer || src->primitive.index.data.ptr) {
		...
		AGL__CALL(glDrawElements(src->primitive.mode, src->primitive.count,
...
void a__GLProgramBind(AGLProgram program,
	...
	for (i = 0; i < nuniforms; ++i) {
		...
		switch(uniforms[i].type) {
			case AGLAT_Float:
				AGL__CALL(glUniform1fv(loc, uniforms[i].count, uniforms[i].value.pf));
				break;
			case AGLAT_Vec2:
				AGL__CALL(glUniform2fv(loc, uniforms[i].count, uniforms[i].value.pf));
				break;
			case AGLAT_Vec3:
				AGL__CALL(glUniform3fv(loc, uniforms[i].count, uniforms[i].value.pf));
				break;
			case AGLAT_Vec4:
				AGL__CALL(glUniform4fv(loc, uniforms[i].count, uniforms[i].value.pf));
				break;
			case AGLAT_Mat2:
				AGL__CALL(glUniformMatrix2fv(loc, uniforms[i].count, GL_FALSE, uniforms[i].value.pf));
				break;
			case AGLAT_Mat3:
				AGL__CALL(glUniformMatrix3fv(loc, uniforms[i].count, GL_FALSE, uniforms[i].value.pf));
				break;
			case AGLAT_Mat4:
				AGL__CALL(glUniformMatrix4fv(loc, uniforms[i].count, GL_FALSE, uniforms[i].value.pf));
				break;
			case AGLAT_Int:
				AGL__CALL(glUniform1iv(loc, uniforms[i].count, uniforms[i].value.pi));
				break;
			case AGLAT_IVec2:
				AGL__CALL(glUniform2iv(loc, uniforms[i].count, uniforms[i].value.pi));
				break;
			case AGLAT_IVec3:
				AGL__CALL(glUniform3iv(loc, uniforms[i].count, uniforms[i].value.pi));
				break;
			case AGLAT_IVec4:
				AGL__CALL(glUniform4iv(loc, uniforms[i].count, uniforms[i].value.pi));
				break;
			case AGLAT_Texture:
...
static void a__GLDepthBind(AGLDepthParams depth) {
	...
	if (cur->mode == AGLDM_Disabled) return;
...
static void a__GLBlendBind(const AGLBlendParams *blend) {
	...
	if (blend->func.src_rgb != cur->func.src_rgb
		...
		if (cur->func.src_rgb == cur->func.src_a && cur->func.dst_rgb == cur->func.dst_a)
			...
			AGL__CALL(glBlendFuncSeparate(cur->func.src_rgb, cur->func.dst_rgb,
...
static void a__GLFramebufferBind(const AGLFramebufferParams *fb) {
	...
	if (a__gl_state.framebuffer.binding != a__gl_state.framebuffer.name)
		AGL__CALL(glBindFramebuffer(GL_FRAMEBUFFER,
	...
	if (fb->depth.mode != AGLDBM_Texture && (depth || color)) {
		...
		AGL__CALL(glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT,
			fb->color->width, fb->color->height));
		AGL__CALL(glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT,
