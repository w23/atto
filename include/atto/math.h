...
typedef struct ALCGRand { uint64_t state_; } ALCGRand;
...
static inline AVec3f aVec3f(float x, float y, float z) {
	const AVec3f r = {x, y, z}; return r;
}

static inline AVec3f aVec3ff(float f) {
	const AVec3f r = {f, f, f}; return r;
}
...
static inline float aVec3fLength2(AVec3f v) { return aVec3fDot(v,v); }
...
static inline AVec4f aVec4f(float x, float y, float z, float w) {
	const AVec4f r = {x, y, z, w}; return r;
}

static inline AVec4f aVec4f3(AVec3f v, float w) {
	const AVec4f r = {v.x, v.y, v.z, w}; return r;
}

static inline AVec4f aVec4ff(float f) {
	const AVec4f r = {f, f, f, f}; return r;
}
...
static inline AMat3f aMat3fv(
		AVec3f x, AVec3f y, AVec3f z) {
	const AMat3f m = {x, y, z}; return m;
}
...
static inline AMat3f aMat3fScale(float s) {
	const AMat3f m = aMat3fv(
			aVec3f(s, 0, 0), aVec3f(0, s, 0), aVec3f(0, 0, s));
	return m;
}

static inline AMat3f aMat3fIdentity() { return aMat3fScale(1.f); }
...
typedef struct AQuat { AVec3f v; float w; } AQuat;

static inline AQuat aQuatIdentity() {
	const AQuat q = {{0, 0, 0}, 1}; return q;
}
...
static inline AQuat aQuatConjugate(AQuat q) {
	const AQuat r = {aVec3fNeg(q.v), q.w}; return r;
}
